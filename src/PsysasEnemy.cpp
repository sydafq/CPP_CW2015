...
PsysasEnemy::PsysasEnemy(PsysasMain* pEngine, int iMapX, int iMapY, TileManager &m_pTileManager)
...
void PsysasEnemy::DoUpdate(int iCurrentTime)
{	
	...
	for (int iObjectId = 0;(pObject = m_pMainEngine->GetDisplayableObject(iObjectId)) != NULL; iObjectId++)
	{
		...
		if (((iXDiff*iXDiff) + (iYDiff*iYDiff))
			< ((iSizeOther + iSize)*(iSizeOther + iSize)) /*&& pointFlag == false*/)
		{
			...
			printf("Collide \n");//Replace with Point system codes
			m_pMainEngine->TotalScore();
		}
	}
